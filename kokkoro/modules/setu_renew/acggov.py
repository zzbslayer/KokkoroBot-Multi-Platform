...
def load_native_info(sub_dir):
    ...
    for fn in fnlist:
        ...
        try:
            ...
            pass
    kokkoro.logger.info(f'[INFO]read{len(info)}setu from{sub_dir}')
    return info
...
async def query_search(keyword):
    ...
    kokkoro.logger.info(f"[INFO]搜索结果数量{len(data['illusts'])}")
    return image_list
...
async def download_acggov_image(url: str):
    kokkoro.logger.info(f'[INFO]acggov downloading image{url}')
    try:
        async with aiohttp.ClientSession(headers=acggov_headers) as session:
...
async def download_pixiv_image(url: str, id):
    kokkoro.logger.info(f'[INFO]acggov downloading pixiv image{url}')
    headers = {
    ...
    while len(image_list) > 0:
        ...
        if os.path.exists(res.path):
            ...
            if get_config('acggov', 'pixiv_direct'):
                ...
                url = url.replace("https://i.pximg.net", "https://i.pixiv.re")
...
async def acggov_fetch_process():
    global ranking_date
    if get_config('acggov', 'mode') == 2:
        kokkoro.logger.info('[INFO]fetch acggov setu')
        for _ in range(10):
        ...
        if date != ranking_date:
            kokkoro.logger.info('[INFO]fetch acggov ranking setu')
            for i in range(25):
