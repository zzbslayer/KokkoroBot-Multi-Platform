...
async def query_setu(r18=0, keyword=None):
	...
	if 'error' not in data:
		return image_list
	if data['error'] != '':
		kokkoro.logger.error(f'[ERROR]lolicon api error,msg:{data["error"]}')
		return image_list
...
async def download_image(url: str):
	kokkoro.logger.info(f'[INFO]lolicon downloading image:{url}')
	try:
		async with aiohttp.ClientSession() as session:
...
async def download_pixiv_image(url: str, id):
	kokkoro.logger.info(f'[INFO]lolicon downloading pixiv image:{url}')
	headers = {
