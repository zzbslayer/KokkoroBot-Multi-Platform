...
if not os.path.exists(pathcfg):
	try:
		with open(pathcfg, 'w') as cfgf:
			...
			kokkoro.logger.error('[WARNING]未找到配置文件，已根据默认配置模板创建，请打开插件目录内config.json查看和修改。')
	except:
		kokkoro.logger.error('[ERROR]创建配置文件失败，请检查插件目录的读写权限及是否存在config.json。')
		traceback.print_exc()
...
if not os.path.exists(glpath):
	try:
		with open(glpath, 'w') as cfggl:
			...
			kokkoro.logger.error('[WARNING]未找到黑白名单文件，已根据默认黑白名单模板创建。')
	except:
		kokkoro.logger.error('[ERROR]创建黑白名单文件失败，请检查插件目录的读写权限。')
		traceback.print_exc()
...
if not os.path.exists(gpcfgpath):
	try:
		with open(gpcfgpath, 'w') as gpcfg:
			...
			kokkoro.logger.error('[WARNING]未找到群个体设置文件，已创建。')
	except:
		kokkoro.logger.error('[ERROR]创建群个体设置文件失败，请检查插件目录的读写权限。')
		traceback.print_exc()
...
# 设置limiter
tlmt = kokkoro.util.DailyNumberLimiter(get_config('base', 'daily_max'))
flmt = kokkoro.util.FreqLimiter(get_config('base', 'freq_limit'))


def check_lmt(uid, num, gid):
	if uid in kokkoro.config.SUPER_USER:
		return 0, ''
...
async def send_msg(msg_list, ev):
	...
	if not get_config('base', 'enable_forward_msg'):
		for msg in msg_list:
			try:
				result_list.append(await kokkoro.bot.get_bot().kkr_send(ev, msg))
			except:
				kokkoro.logger.error('[ERROR]图片发送失败')
				await kokkoro.bot.get_bot().kkr_send(ev, f'涩图太涩,发不出去力...')
			await asyncio.sleep(1)
	else:
		forward_msg = render_forward_msg(msg_list)
		try:
			result_list.append(await kokkoro.bot.get_bot().kkr_send_by_group(ev.get_group_id(), forward_msg))
		except:
			traceback.print_exc()
			kokkoro.logger.error('[ERROR]图片发送失败')
			await kokkoro.bot.get_bot().kkr_send(ev, f'涩图太涩,发不出去力...')
...
async def send_setu(bot, ev):
	...
	if not is_su:
		...
		if args[1] in ["新增", "添加"]:
			...
			await bot.kkr_send(ev,"操作错误，应为新增/删除其一")
			return
		group_id = args[2]
...
async def send_search_setu(bot, ev):
	...
	if num:
		try:
			...
			if num > max_num:
				await bot.kkr_send(ev, f'太贪心辣,一次只能要{max_num}份涩图哦~')
				num = max_num
			else:
				pass
		except:
	...
	if result != 0:
		await bot.kkr_send(ev, msg)
		return
	keyword = ev.get_param().match.group('keyword')
	result_list = []
	msg_list = []
	if not keyword:
		for _ in range(num):
	...
	if second and second > 0:
		...
		for result in result_list:
			try:
				...
				traceback.print_exc()
				kokkoro.logger.error('[ERROR]撤回失败')
			await asyncio.sleep(1)
...
async def send_ranking(bot, ev):
	gid = ev.get_group_id()
	page = ev.get_param().match.group(1)
...
async def get_spec_setu(bot, ev):
	...
	try:
		...
		await bot.kkr_send(ev, '请在命令之后提供p站id哦~')
		return
	args = str(args)
	if len(args) == 8:
		msg = get_spec_image(args)
		if not msg:
			await bot.kkr_send(ev, f'没有在本地找到这张图片/不支持r18图片的提取\n原图地址:https://pixiv.lancercmd.cc/{args}')
		else:
			await bot.kkr_send(ev, msg)
	else:
		await bot.kkr_send(ev, 'p站id无效,应为8位数字id哦~')
