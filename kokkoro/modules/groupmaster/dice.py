...
async def do_dice(bot: KokkoroBot, ev: EventInterface, num, min_, max_, opr, offset, TIP="的掷骰结果是："):
    if num == 0:
        await bot.kkr_send(ev, '咦？我骰子呢？')
        return
...
async def dice(bot: KokkoroBot, ev: EventInterface):
...
@sv.on_prefix('.qj')
async def kc_marriage(bot: KokkoroBot, ev: EventInterface):
    wife = ev.get_param().remain
